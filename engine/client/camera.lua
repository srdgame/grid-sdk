...
_minZoom = _minZoom or 1 * love.window.getDPIScale()

function getMinZoom()
	return _minZoom
end

_maxZoom = _maxZoom or 4 * love.window.getDPIScale()

function getMaxZoom()
	return _maxZoom
end

_zoom = _zoom or 1 * love.window.getDPIScale()
...
function resetZoom()
	if ( _tween == nil ) then
		_tween = tween( _M, nil, {
			_zoom      = 2 * love.window.getDPIScale(),
